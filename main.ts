...
// --- Bing Maps ---
const bingMapsKey = env.BING_MAPS_KEY;

async function bingMapsGetLocation(query: string): Promise<{ lat: number, lon: number, address: string } | { "status": number, "error": string }> {
  const maxResults = 1;
